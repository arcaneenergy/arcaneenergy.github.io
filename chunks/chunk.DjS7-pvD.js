...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./chunk.DodmLh3E.js');
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./chunk.DP96Yw-1.js').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./chunk.Dup3owUt.js');
